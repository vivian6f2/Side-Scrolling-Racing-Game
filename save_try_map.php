<?php
	...
	$sql = "INSERT INTO project_TRY_MAP(id, name, rate, rate_time, time)
			VALUES(NULL, ?, ?, ?, ?)";
	$sth = $db->prepare($sql);
	$sth->execute(array($map_name, 3.0, 1, $time));
	//get map id
	$sql = "SELECT * FROM project_TRY_MAP WHERE name = ? AND time = ?";
	$sth = $db->prepare($sql);
	...
	foreach ($decode_map as $mp){
		$sql = "INSERT INTO project_TRY_MAP_DATA(id, mid, x, y, item)
				VALUES(NULL, ?, ?, ?, ?)";
		$sth = $db->prepare($sql);
	...
	foreach ($decode_map as $mp){
		$sql = "INSERT INTO project_TRY_MAP_OBJECT(id, mid, x, y, item)
				VALUES(Null, ?, ?, ?, ?)";
		$sth = $db->prepare($sql);
		$sth->execute(array($mid, $mp->x, $mp->y, $mp->item));
	}
