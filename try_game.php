...
<?php
	...
	$sql = "SELECT * FROM project_TRY_MAP_DATA WHERE mid = ? ORDER BY x DESC";
	$sth = $db->prepare($sql);
	$sth->execute(array($mid));
	$row = $sth->fetchObject();
	$max_x = $row->x;
	...
	$sql = "SELECT * FROM project_TRY_MAP_OBJECT WHERE mid = ? ORDER BY x DESC";
	$sth = $db->prepare($sql);
	$sth->execute(array($mid));
	$row = $sth->fetchObject();
	...
	$sql = "SELECT * FROM project_TRY_MAP_OBJECT WHERE mid = ? AND item = 4";
	$sth = $db->prepare($sql);
	$sth->execute(array($mid));
	$row = $sth->fetchObject();
	$flag_x = $row->x;
...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			function reset(){
				...
				//document.all.scoreButton1.style.display="initial";
				//document.all.scoreButton2.style.display="initial";
				/*for(i=1;i<=10;i++){
			...
			function distanceBar(){
				...
				if(map_flag_x < middle_char_x) document.getElementById("small_char").src="image/char_move_left.gif";
				else document.getElementById("small_char").src="image/char_move_right.gif";
	...
	<body onkeydown="keydownFunction(event)" onkeyup="keyupFunction(event)" onload="init()" onresize="ifResize()">
		<div id="winPage">
			<h3>You Win!</h3>
			<!--<br>
			...
			<div>
			<?php
				echo '<a href="create_map.php?mid='.$mid.'" target="_self" class="backNoFloat" style="position:absolute; top:200px; left:50%; margin-left:-60px;">.</a>';
			?>
		...
		<header>
			<?php
				echo '<a href="create_map.php?mid='.$mid.'" target="_self" class="back">B</a>';
				echo '<h1>'.$map_name.'</h1>';
			?>
